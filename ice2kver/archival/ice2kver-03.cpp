...
int main(int argc, char *argv[]) {
  ...
  FXIcon* bannericon = new FXGIFIcon(&application, banner03,0,IMAGE_OPAQUE);
  // FXIcon* bannericon = new FXGIFIcon(&application, banner2k,0,IMAGE_OPAQUE);

  application.init(argc, argv);
  FXMainWindow *main=new FXMainWindow(&application, "About Windows", NULL, NULL, DECOR_TITLE|DECOR_BORDER|DECOR_MENU|DECOR_CLOSE, 0, 0, 413, 322);

  new FXLabel(main, "", bannericon, 0, 0, 0, 0, 0, 0, 0, 0);
  ...
  // FXFrame* eulaunderscore1 = new FXFrame(eulacont1, LAYOUT_FIX_WIDTH|LAYOUT_FIX_HEIGHT|LAYOUT_FIX_X|LAYOUT_FIX_Y, eulalink1->getX(), eulalink1->getY() + eulalink1->getDefaultHeight() - 5, eulalink1->getDefaultWidth(), 5, 0, 0, 0, 0);
  ...
  //new FXLabel(winverinfo, "License Agreement to:", NULL, 0, 0, 0, 0, 0, 0, 0, 0, 5);
  ...
  FXPacker *okcont=new FXPacker(main,LAYOUT_SIDE_BOTTOM|LAYOUT_RIGHT, 0, 0, 0, 0, 10, 8, 10, 10);

  FXButton* okbtn = new FXButton(okcont, "OK", NULL, &application, FXApp::ID_QUIT, BUTTON_NORMAL|LAYOUT_FIX_WIDTH|LAYOUT_FIX_HEIGHT|BUTTON_DEFAULT, 0, 0, 75, 23, 3, 3, 2, 3);
  okbtn->setFocus();
  ...
  // set window on top
  //printf("%p", (int*)ptrapp->getDisplay());
  //printf("\n");
  x11_window_set_on_top((Display*)ptrapp->getDisplay(), main->id());
