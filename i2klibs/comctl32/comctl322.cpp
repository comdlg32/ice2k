...
void I2KButton::drawBorderRectangle(FXDCWindow& dc,FXint x,FXint y,FXint w,FXint h) {
// void FXFrame::drawBorderRectangle(dc,x,y,w,h) {
...
void I2KButton::drawBorderRectangle(FXDCWindow& dc,FXint x,FXint y,FXint w,FXint h) {
  if (focuscolor == 0) {
    focuscolor = getApp()->reg().readColorEntry("SETTINGS", "focuscolo	r", FXRGB(0, 0, 0));
  }
...
FXIMPLEMENT(I2KMenuButton,FXMenuButton,I2KMenuButtonMap,ARRAYNUMBER(I2KMenuButtonMap))

//I2KMenuButton::I2KMenuButton(FXComposite* p) : FXMenuButton(p) {
//}

I2KMenuButton::I2KMenuButton(FXComposite* p,const FXString& text,FXIcon* ic,FXPopup* pup,FXuint opts,FXint x,FXint y,FXint w,FXint h,FXint pl,FXint pr,FXint pt,FXint pb):
  ...
  // Draw arrows
    ...
    if((options&MENUBUTTON_RIGHT)==MENUBUTTON_RIGHT){
      if(isEnabled())
        ...
        dc.setForeground(shadowColor);
      points[0].x=ix+1; // CHECK ARROW CODE HERE
      points[0].y=iy;
      points[1].x=ix+1;
      points[1].y=iy+MENUBUTTONARROW_WIDTH-1 - 2;
      points[2].x=ix+MENUBUTTONARROW_HEIGHT - 1;
