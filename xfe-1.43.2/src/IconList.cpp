...
void IconItem::drawDetails(IconList* list, FXDC& dc, int x, int y, int, int h) const
{
    ...
    if (isSelected())
    {
        ...
        dc.setForeground(list->getSelBackColor());
        //dc.fillRectangle(x, y, header->getTotalSize(), h);
        //dc.fillRectangle(x, y, font->getTextWidth(label.text(), label.length()), h);
        //dc.fillRectangle(x, y, 1, h);
        int itemWidth;
        int textWidth = font->getTextWidth(label.section('\t', 0));
        int colWidth = header->getItemSize(0) - iw - 9;
	  ...
	  //system("xterm");
        dc.fillRectangle(iw +  4 + x, y + 1, 4 + itemWidth, h - 2);
	//printf("%s\n", label.section('\t', 0).text());
    }
    if (hasFocus())
    {
        if (miniIcon)
        ...
        dc.drawFocusRectangle(iw +  4 + x, y + 1, 4 + itemWidth, h - 2);
    ...
    if (miniIcon)
    {
        ...
        //dc.drawIcon(miniIcon, xx+2, y+(h-ih)/2);
    ...
    if (!label.empty())
    {
        ...
        for (hi = 0, beg = 0; beg < label.length() && hi < header->getNumItems(); hi++, beg = end+1)
        {
            ...
            if (end > beg)
            {
                ...
                {
                	// If selected, set name column have selected text color but the rest normal text
                	if (hi == 0)
                	{
                    		if (tw > space-4)
                    		{
    ...
    FXScrollArea(p, opts, x, y, w, h)
{
    ...
    int screenres = getApp()->reg().readIntEntry("SETTINGS", "screenres", 96);

    if (screenres == 100) {
        screenres = 96;
    }

    fixed_item_space = (float)76 / (float)96 * (float)screenres;

    //itemSpace = ITEM_SPACE;
    itemSpace = fixed_item_space;
