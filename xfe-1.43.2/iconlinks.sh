...
link_theme4=windowsxp-theme

echo Installing icon links...

# Loop on GIF and PNG files

for ext in gif png
do
	for iconfile in $1/icons/$ref_theme1/*.$ext
	do
		...
		then
			echo ln -s -f ../$ref_theme3/$iconname $2/icons/$link_theme4/$iconname;
			ln -s -f ../$ref_theme3/$iconname $2/icons/$link_theme4/$iconname
		fi

		if test ! -f $1/icons/$link_theme4/$iconname
		then
			echo ln -s -f ../$ref_theme1/$iconname $2/icons/$link_theme4/$iconname;
			ln -s -f ../$ref_theme1/$iconname $2/icons/$link_theme4/$iconname
		fi
	done
done
